...
MERGED_DIR = WORK_DIR / "merged_dataset_2"
EXTRACT_DIR = WORK_DIR / "datasets"

# unified 5 target classes (fire and smoke merged into fire_smoke)
TARGET_CLASSES = [
    'person',
    'fire_smoke',
    'exit_indicator',
...
def prepare_dataset():
    ...
    print("[2/4] Parsing and converting COCO JSON to merged_dataset_2...")
...
def train_and_export():
    print("[3/4] Starting YOLOv8 Training for merged classes...")
    ...
    results = model.train(
        ...
        close_mosaic=10,
        project=str(WORK_DIR / 'runs_2'),
        name='scout_disaster_v2',
        exist_ok=True
    )

    print("[4/4] Exporting ONNX models...")
    weights_dir = WORK_DIR / 'runs_2' / 'scout_disaster_v2' / 'weights'
    ...
    final_onnx_640 = weights_dir / 's2m_vAI_lite_640_v2.onnx'
    ...
    final_onnx_320 = weights_dir / 's2m_vAI_lite_320_v2.onnx'
    if Path(exported_320_path).exists():
        shutil.move(exported_320_path, final_onnx_320)

    # generate labels.txt
    labels_file = WORK_DIR / 's2m_vAI_lite_labels_v2.txt'
    with open(labels_file, 'w', encoding='utf-8') as f:
        for name in TARGET_CLASSES:
            f.write(f"{name}\n")

    print("\n==========================================")
    print("Training and Export Finished (Version 2)!")
