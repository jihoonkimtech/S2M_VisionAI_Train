...
MERGED_DIR = WORK_DIR / "merged_dataset_6"
...
# exclude fire-smoke-detection.zip
ZIP_FILES = [
...
DATASET_RULES = {
    ...
    'fire-and-person-detection': lambda name: (
        ...
        else ('smoke' if 'smoke' in name.lower() else ('fire' if 'fire' in name.lower() else None))
    ),
}
# =================================================
...
def prepare_dataset():
    ...
    print("[2/4] Parsing and converting COCO JSON to merged_dataset_6 (6 classes)...")
...
def train_and_export():
    print("[3/4] Starting YOLOv8 Training for separated 6 classes (v6)...")
    ...
    results = model.train(
        ...
        mixup=0.15,
        close_mosaic=10,
        project=str(WORK_DIR / 'runs_6'),
        name='scout_disaster_v6',
        exist_ok=True
    )

    print("[4/4] Exporting ONNX models...")
    weights_dir = WORK_DIR / 'runs_6' / 'scout_disaster_v6' / 'weights'
    ...
    final_onnx_640 = weights_dir / 's2m_vAI_lite_640_v6.onnx'
    ...
    final_onnx_320 = weights_dir / 's2m_vAI_lite_320_v6.onnx'
    if Path(exported_320_path).exists():
        shutil.move(exported_320_path, final_onnx_320)

    # generate labels.txt
    labels_file = WORK_DIR / 's2m_vAI_lite_labels_v6.txt'
    with open(labels_file, 'w', encoding='utf-8') as f:
        for name in TARGET_CLASSES:
            f.write(f"{name}\n")

    print("\n==========================================")
    print("Training and Export Finished (Version 6 - 6 Classes)!")
