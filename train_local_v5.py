...
MERGED_DIR = WORK_DIR / "merged_dataset_5"
EXTRACT_DIR = WORK_DIR / "datasets"

# 5 target classes (fire and smoke merged)
TARGET_CLASSES = [
    'person',
    'fire_smoke',
    'exit_indicator',
...
def prepare_dataset():
    ...
    print("[2/4] Parsing and converting COCO JSON to merged_dataset_5 (5 classes)...")
...
def train_and_export():
    print("[3/4] Starting YOLOv8 Training for merged classes (v5)...")
    ...
    results = model.train(
        ...
        mixup=0.15,
        close_mosaic=10,
        project=str(WORK_DIR / 'runs_5'),
        name='scout_disaster_v5',
        exist_ok=True
    )

    print("[4/4] Exporting ONNX models...")
    weights_dir = WORK_DIR / 'runs_5' / 'scout_disaster_v5' / 'weights'
    ...
    final_onnx_640 = weights_dir / 's2m_vAI_lite_640_v5.onnx'
    ...
    final_onnx_320 = weights_dir / 's2m_vAI_lite_320_v5.onnx'
    if Path(exported_320_path).exists():
        shutil.move(exported_320_path, final_onnx_320)

    # generate labels.txt
    labels_file = WORK_DIR / 's2m_vAI_lite_labels_v5.txt'
    with open(labels_file, 'w', encoding='utf-8') as f:
        for name in TARGET_CLASSES:
            f.write(f"{name}\n")

    print("\n==========================================")
    print("Training and Export Finished (Version 5 - 5 Classes)!")
