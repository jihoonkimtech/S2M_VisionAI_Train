...
MERGED_DIR = WORK_DIR / "merged_dataset_3"
EXTRACT_DIR = WORK_DIR / "datasets"

# unified 5 target classes
TARGET_CLASSES = [
    'person',
    'fire_smoke',
    'exit_indicator',
...
# exclude fire-and-person-detection.zip to eliminate conflicting annotations
ZIP_FILES = [
...
# clean rule mapping without fire-and-person dataset
DATASET_RULES = {
    ...
    'fire-smoke-detection': lambda name: 'fire_smoke',
}
# =================================================
...
def prepare_dataset():
    ...
    print("[2/4] Parsing and converting COCO JSON to merged_dataset_3...")
...
def train_and_export():
    print("[3/4] Starting YOLOv8 Training for merged classes (v3)...")
    ...
    results = model.train(
        ...
        optimizer='AdamW',
        hsv_h=0.015,
        hsv_s=0.7,         # enhanced saturation for smoke/flame
        hsv_v=0.4,
        scale=0.5,
        mosaic=1.0,
        mixup=0.15,        # mixup augmentation for semi-transparent smoke
        close_mosaic=10,
        project=str(WORK_DIR / 'runs_3'),
        name='scout_disaster_v3',
        exist_ok=True
    )

    print("[4/4] Exporting ONNX models...")
    weights_dir = WORK_DIR / 'runs_3' / 'scout_disaster_v3' / 'weights'
    ...
    final_onnx_640 = weights_dir / 's2m_vAI_lite_640_v3.onnx'
    ...
    final_onnx_320 = weights_dir / 's2m_vAI_lite_320_v3.onnx'
    if Path(exported_320_path).exists():
        shutil.move(exported_320_path, final_onnx_320)

    # generate labels.txt
    labels_file = WORK_DIR / 's2m_vAI_lite_labels_v3.txt'
    with open(labels_file, 'w', encoding='utf-8') as f:
        for name in TARGET_CLASSES:
            f.write(f"{name}\n")

    print("\n==========================================")
    print("Training and Export Finished (Version 3)!")
