...
MERGED_DIR = WORK_DIR / "merged_dataset_9"
...
# removed noisy dataset to improve accuracy
ZIP_FILES = [
...
# safe mappings without dropping boxes
...
def prepare_dataset():
    ...
    print("[2/4] Parsing and converting datasets to merged_dataset_9 (7 classes)...")
        ...
        for split in ['train', 'valid', 'val', 'test']:
            ...
            if json_files:
                ...
                for img_id, img_info in images.items():
                    # downsample people dataset
                    # prevents class imbalance
                    if folder.name == 'people' and tgt_split == 'train' and random.random() > 0.35:
                        continue
            ...
            # check for standard yolo format
            ...
            for src_img in img_dir.iterdir():
                ...
                # downsample people dataset
                # reduces false positives on background
                if folder.name == 'people' and tgt_split == 'train' and random.random() > 0.35:
                    continue
...
def train_and_export():
    print("\n[3/4] Starting YOLOv8 Training (Version 9 - Clean Dataset & Smoke/Overlap Tuned)...")
    
    # train from scratch using nano model
    # ignore previous weights to avoid bias
    print("[*] Base model: yolov8n.pt (Training from scratch with clean dataset)")
    ...
    results = model.train(
        ...
        lr0=0.001,
        # tuned loss weights
        # balances classification and overlap separation
        box=8.5,
        cls=1.2,
        dfl=2.0,
        hsv_h=0.015,
        ...
        copy_paste=0.15,
        close_mosaic=15,
        project=str(WORK_DIR / 'runs_9'),
        name='scout_disaster_v9',
        exist_ok=True
    )

    print("\n[4/4] Exporting ONNX models...")
    weights_dir = WORK_DIR / 'runs_9' / 'scout_disaster_v9' / 'weights'
    best_weights = weights_dir / 'best.pt'
    trained_model = YOLO(str(best_weights))

    # export onnx standard size
    ...
    final_onnx_320 = weights_dir / 's2m_vAI_lite_320_v9.onnx'
    if Path(exported_320_path).exists():
        shutil.move(exported_320_path, final_onnx_320)

    labels_file = WORK_DIR / 's2m_vAI_lite_labels_v9.txt'
    with open(labels_file, 'w', encoding='utf-8') as f:
        for name in TARGET_CLASSES:
            f.write(f"{name}\n")

    print("\n==========================================")
    print("Training and Export Finished (V9 Complete)!")
